...
public class Game extends CharTimeSpace implements Runnable
{
	...
	short missileTraceScreenRes[];
	
	public PlayersJetCamera mainCamera;
	public PlayersJetCamera  missileTraceCamera;
	...
	public CharDynamicHUD		missileTraceScreen;
	...
	)
	{
		...
		*/
		...
		missileTraceScreenRes = new short[2];
		missileTraceScreenRes[0] = (short)30;
		missileTraceScreenRes[1] = (short)15;
		
		missileTraceScreen = visualManager.newDynamicHUD(null, (short)999, missileTraceScreenRes[0], missileTraceScreenRes[1], (short)15, (short)35);
		missileTraceCamera = new PlayersJetCamera
		(
			3.0, visibility, 
			visibility * 10, (short)0, (short)100, 
			missileTraceScreenRes,
			missileTraceScreen.HUDImg,
			this, myJet, 
			new CharDynamicHUD
			(
				hud_file5,
				missileTraceScreen.HUDImg,
				(short)0, missileTraceScreenRes,
				(short)6, (short)5
			), 
			new CharDynamicHUD
			(
		...
		missileTraceCamera.mainCamera = false;
		missileTraceCamera.connectLocationAndAngle(mainCamera.location, mainCamera.roll_angle);
		visualManager.newCamera(missileTraceCamera);
		missileTraceCamera.enabled = false;
			...
			while(!eventManager.EventFrapsQueue_keyboard.isEmpty())
			{
				...
				switch(keyPressed)
				{
					...
					case 16386:
					{
						...
						if(m != null) mainCamera.connectLocationAndAngle(m.location, m.cameraRollAngle);
					}
					break;
					
					case 16387:
					{
						missileTraceCamera.enabled = true;
						Missile m = myJet.missileOpenFire(false, lockedEnemy, missileTraceCamera);
						if(m != null) missileTraceCamera.connectLocationAndAngle(m.location, m.cameraRollAngle);
					}
					break;
